...
export default function DoctorMessagesPage() {
  ...
  const { data: sent } = useSWR(
    doctorId ? ["messages-sent", doctorId] : null,
    async () => {
  ...
  // Combine and filter messages for selected patient
  const conversation = [...(Array.isArray(inbox) ? inbox : []), ...(Array.isArray(sent) ? sent : [])]
    .filter((msg) =>
      selectedPatientId
        ? msg.sender_id === selectedPatientId || msg.recipient_id === selectedPatientId
        : false
    )
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">Comunícate con tus pacientes</p>
        </div>
        {totalUnread > 0 && (
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-1">
          <CardHeader>
            <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Pacientes</h2>
          </CardHeader>
          <CardBody className="max-h-[500px] overflow-y-auto p-0">
            {loadingPatients ? (
              <div className="flex justify-center py-4"><Spinner /></div>
            ) : !patients || patients.length === 0 ? (
              <p className="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">No tienes pacientes aún</p>
            ) : (
              <div className="divide-y divide-gray-100 dark:divide-gray-700">
                {patients.map((p: DoctorPatient) => (
                  <button
                    key={p.id}
                    ...
                    className={`flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50 ${
                      selectedPatientId === p.id ? "bg-primary-50 dark:bg-primary-900/20" : ""
                    }`}
                  >
                    ...
                    {unreadBySender.has(p.id) && (
                      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white">
                        {unreadBySender.get(p.id)}
                      </span>
                    )}
                  </button>
                ))}
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
              {selectedPatientId
                ? `Conversación con ${patients?.find((p: DoctorPatient) => p.id === selectedPatientId)?.first_name || "Paciente"}`
                ...
                <div className="flex-1 space-y-3 overflow-y-auto max-h-[350px] mb-4">
                  {conversation.length === 0 ? (
                    ...
                    conversation.map((msg) => (
                      <div
                        key={msg.id}
                        className={`flex ${msg.sender_id === doctorId ? "justify-end" : "justify-start"}`}
                      >
                        <div
                        ...
                        >
                          ...
                          <p className={`mt-1 text-xs ${msg.sender_id === doctorId ? "text-primary-200" : "text-gray-400"}`}>
